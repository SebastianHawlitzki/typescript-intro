...
// Objekte können ohne Klassen definiert werden -> User-Object / type definiert wie das Objekt aussehen soll
type User = {
    name: string | null,
    age: number | null
    pets: string | null [],
    sayName(): string | null // es darf entweder ein String sein oder Null u, flexibel zu bleiben und werte auszulassen
};

//Initialisierung des Objektes User
function objectTypes() {
    ...
    //  user.name = "Hello"; // name kann überschrieben werden
...
//todo
        const shortAnimals = animals.filter(animal => {
            return animal.length < 5; //animals unter 5 zeichen mit filter
        });

        const repeatAnimals = animals.map(animal => animal.repeat(2));
        console.log("repeat animals", repeatAnimals) //wiederholt animals 2mal


        const characterCount = animals.reduce(
            (sum, current) => sum + current.length, 0
        );
        ...
        for (let animal of animals) {
            console.log(animal); //Liste ausgeben
        }
...
function studentTypes() {
    ...
    //update by ID / funktion die einen Studenten im Array findet und Namen updated

    function updateById(id: number) {
        const newStudent: Student | undefined = students.find(students => {
            id === students.id
        })
    ...
    // verändere die function so, dass das ursprüngliche Objekt nicht verändert wird
...
//spread operator  ...student -> kopiert Student und dann kann mit z.b name: Name , der Name überschrieben werden
// conat arr = [], const arr2 = [] const total arr = [...arr, arr2..]



// Rick and Morty Aufgabe
// schreibe eine Function die ein Array mit lebenden Menschen erstellt

function aliveAndHumanCharacters () {
  const name = data.results.filter(character => {
return character.name && character.status
    })


}

// schreibe ein function die ein Array mit allen Namen zurück gibt
function characterWithNames () {
